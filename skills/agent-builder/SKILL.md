...
**Implementation (Go)**:
- `references/minimal-agent.go` - Complete working agent (~120 lines)
- `references/tool-templates.go` - Capability definitions & implementations
- `references/subagent-pattern.go` - Context isolation with agent type registry

**Scaffolding**:
- `scripts/init_agent.go` - Generate new agent projects (`go run init_agent.go <name> -level 0|1`)
