...
govulncheck ./...            # Go (install: go install golang.org/x/vuln/cmd/govulncheck@latest)
npm audit                    # Node.js
cargo audit                  # Rust
grep -r "password\|secret\|api_key" --include="*.go" --include="*.yaml"
```
...
exec.Command("bash", "-c", "ls "+userInput).Run()
// Good:
...
// Bad: Resource leak - unclosed body
resp, _ := http.Get(url)
// Good:
...
grep -rn "password\|secret\|token" . --include="*.go" --include="*.yaml"
...
# Check dependencies
go list -m -u all                  # List outdated Go modules
npm outdated                       # Node
```
